...
import { useActiveAddress, useConnection, useStrategy } from '@arweave-wallet-kit/react'
...
export default function Layout({ children }) {
  ...
  const [address, selectedDrive, login, syncAllUserDrives, createDrive] = useGlobalStore((state) => [
  ...
  if (!address) {
    return (
      <div className="flex min-h-screen w-screen bg-gray-50 items-center justify-center ">
        <button onClick={connect} className="bg-blue-500 text-white px-4 py-2 rounded-md">
          Connect Wallet
        </button>
      </div>
    )
